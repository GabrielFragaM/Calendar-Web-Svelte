        ...
        items = [
            {
                ...
                start: new Date(y, m, randInt(6)),
                end: new Date(y, m, randInt(6) + randInt(4)),
            },
            {
                title: "7:30 Wk 2 tasks",
                className: "task--warning",
                start: d1,
                end: new Date(y, m, d1.getDate() + randInt(4) + 1),
            },
            {
                title: "Overlapping Stuff (isBottom:true)",
                start: d1,
                end: new Date(y, m, d1.getDate() + 3),
                className: "task--info",
                isBottom: true,
            },
            {
                title: "10:00 More Stuff to do",
                start: new Date(y, m, randInt(7) + 14),
                end: new Date(y, m, randInt(7) + 14 + randInt(4)),
                className: "task--info",
            ...
            {
                title: "All day task",
                start: new Date(y, m, randInt(7) + 21),
                end: new Date(y, m, randInt(7) + 21),
                className: "task--danger",
                vlen: 2,
            },
        ];

        // Calculate the length (len) based on start and end dates
        for (let i of items) {
            i.len = (i.end - i.start) / (1000 * 60 * 60 * 24) + 1;
        }

        // This is where you calc the row/col to put each dated item
        for (let i of items) {
            let rc = findRowCol(i.start);
            if (!rc) {
